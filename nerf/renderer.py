...
class NeRFRenderer(nn.Module):
    ...
    def run_patch(self, rays_patch_o, rays_patch_d, num_steps=64, upsample_steps=192, epoch=0, bg_color=None, perturb=True):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # bg_color: [3] in range [0, 1]
        # return: image: [B, N, 3], depth: [B, N]
        # rays_o, rays_d: [B, N, PS * 2 + 1, PS * 2 + 1, 3]
        ...
        # sample steps
        nears, fars = raymarching.near_far_from_aabb(rays_center_o, rays_center_d, aabb, self.min_near)
        nears.unsqueeze_(-1)
        fars.unsqueeze_(-1)
        ...
        offulr = torch.rand(N, num_steps, device=device)[:, None, :]
        offdlr = torch.rand(N, num_steps, device=device)[:, None, :]
        offud = torch.rand(N, num_steps, device=device)[:, None, :]
        dir_ulr = (1.0 - offulr) * rays_ul_d + offulr * rays_ur_d
        dir_dlr = (1.0 - offdlr) * rays_dl_d + offdlr * rays_dr_d
        rays_d = (1.0 - offud) * dir_ulr + offud * dir_dlr

        rcd = rays_center_d.unsqueeze(dim=-1).expand((prefix[0]*prefix[1], 3, num_steps))
        sin_add = torch.norm(torch.cross(rays_d, rcd, dim=-2), dim=-2)
        z_vals = torch.sqrt_(torch.square(z_vals) + torch.square(sin_add))
        # generate xyzs
        rays_d = rays_d.permute(0, 2, 1).contiguous()
        xyzs = rays_center_o.unsqueeze(-2) + rays_d * z_vals.unsqueeze(-1)  # [N, 1, 3] * [N, T, 1] -> [N, T, 3]
        ...
        if upsample_steps > 0:
            ...
            new_rays_d = new_rays_d.permute(0, 2, 1).contiguous()
            with torch.no_grad():
            ...
            rays_d = torch.cat([rays_d, new_rays_d], dim=1)
            rays_d = torch.gather(rays_d, dim=1, index=z_index.unsqueeze(-1).expand_as(rays_d))
        ...
        if self.bg_radius > 0:
            ...
            sph = raymarching.sph_from_ray(rays_center_o, rays_d, self.bg_radius)  # [N, 2] in [-1, 1]
    ...
    def run_depth(self, rays_o, rays_d, num_steps=64, upsample_steps=192, epoch=0, bg_color=None, perturb=False, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # bg_color: [3] in range [0, 1]
        # return: image: [B, N, 3], depth: [B, N]
